...
### An OpenStack cluster with Manila CSI

In this example, we deploy a k8s cluster with the full OpenStack integration
plus Manila CSI for shared-filesystem storage. This adds to the model:
* openstack-integrator (trusted)
* cinder-csi
* openstack-cloud-controller
* csi-driver-nfs
...
`csi-driver-nfs` and `manila-csi` are subordinate charms — they deploy onto
the k8s-worker units, so no units, base, or constraints are needed for them.
...
To use this, set both `cloud_integration` and `csi_integration` in your `main.tf`:
